...
export function validDiscoveryRequest(value) {
  return exactKeys(value, ["schema_version", "request_id", "idempotency_key", "host_capabilities"])
    && value.schema_version === "1" && validId(value.request_id) && IDEMPOTENCY_KEY.test(value.idempotency_key || "")
    && validHostCapabilities(value.host_capabilities);
}
...
export async function createSignedManifest({ packageRecord, catalogRevision, assemblyReceipt, request, signingPrivateKey, issuedAt }) {
...
export function noPackageResponse(encounterId, requestId) {
  return { schema_version: "1", status: "no_package", encounter_id: encounterId, request_id: requestId, reason: "no_accepted_compatible_package" };
}

function validHostCapabilities(value) {
