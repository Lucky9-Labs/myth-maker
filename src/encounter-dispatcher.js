...
function validateEvents(order, events) { if (!Array.isArray(events) || !events.length) throw new InvalidWorkerEventsError("backend returned no WorkerEvents"); let worker, sequence = -1, state = "dispatching"; for (const event of events) { if (!event || event.schema_version !== "1" || !ID.test(event.event_id) || event.work_id !== order.work_id || event.encounter_id !== order.encounter_id || !ID.test(event.worker_id) || !Number.isInteger(event.sequence) || event.sequence <= sequence || typeof event.occurred_at !== "string") throw new InvalidWorkerEventsError("backend event has invalid v1 identity or sequence"); if (worker && worker !== event.worker_id) throw new InvalidWorkerEventsError("backend events must bind one worker"); worker ||= event.worker_id; sequence = event.sequence; if (state === "dispatching" && event.kind === "accepted") state = "accepted"; else if (state === "accepted" && event.kind === "started") state = "running"; else if (state === "running" && ACTIVE.has(event.kind)) state = "running"; else if (["dispatching", "accepted", "running"].includes(state) && TERMINAL.has(event.kind)) state = "terminal"; else throw new InvalidWorkerEventsError("backend event lifecycle is illegal"); } if (state !== "terminal") throw new InvalidWorkerEventsError("backend events must end terminally"); }
