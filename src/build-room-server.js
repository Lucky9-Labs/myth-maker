...
export function createBuildRoomServer({ room = new BuildRoom(), persist = () => {}, catalog = createSqliteCatalog() } = {}) {
  ...
  room.setCatalogProjection(() => catalog.buildRoomInventoryProjection?.() || catalog.projectionSummary());
...
<body>
  ...
  <p class="muted">Local inspection surface. Submission runs the local planner, dispatcher, and local worker processes; Cloudflare, Modal, Blender, and Unity remain absent unless receipt-bearing evidence arrives.</p>
  <div class="notice"><strong>Evidence boundary:</strong> local HTTP and worker receipts are observed locally; catalog counters are direct local SQLite reads; Cloudflare, Modal, and Blender remain absent until receipt-bearing adapter input arrives.</div>
...
const CLIENT_SCRIPT = String.raw`
  ...
  function inventoryRows(projection) { const rows = projection?.items || []; return rows.length ? "<table><thead><tr><th>Stable ID</th><th>Domain</th><th>Source → runtime</th><th>Evidence</th><th>Preview</th></tr></thead><tbody>" + rows.map((row) => "<tr><td><code>" + esc(row.stable_id) + "</code></td><td>" + esc(row.domain) + " rev " + esc(row.revision) + "</td><td>" + esc(row.source_format) + " → " + esc(row.runtime_format) + " (" + esc(row.runtime_state) + ")</td><td>" + esc(row.evidence_tier) + "</td><td>" + esc(row.thumbnail_state) + "</td></tr>").join("") + "</tbody></table>" : "<p class=\"muted\">No seeded catalog entries are connected.</p>"; }
