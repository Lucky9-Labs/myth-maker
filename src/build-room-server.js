...
const CLIENT_SCRIPT = String.raw`
  ...
  function workerLanes(run, elapsed) {
    ...
    return workItems.map((work) => { const workElapsed = Math.max(0, Math.floor((Date.now() - Date.parse(work.started_at)) / 1000)); const edges = work.depends_on_work_ids.length ? work.depends_on_work_ids.map((dependency) => dependency + " → " + work.work_id).join("; ") : "request → " + work.work_id; return "<div class=\"lane " + evidenceClass(work.evidence_kind) + "\"><strong>Planner → local dispatcher → " + esc(work.lane) + "</strong><br>work: <code>" + esc(work.work_id) + "</code><br>worker: <code>" + esc(work.worker_id) + "</code><br>status: " + esc(work.status) + " · " + workElapsed + "s<br>dependency edges: " + esc(edges) + "<br>evidence: " + esc(labels[work.evidence_kind]) + "</div>"; }).join("");
  }
