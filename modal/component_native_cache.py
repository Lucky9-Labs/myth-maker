...
def run_component_native_cache(job:dict,submissions_root:Path,blender:str)->dict:
    ...
    cp=subprocess.run([blender,"--background","--factory-startup","--disable-autoexec","--python","/opt/component_native_cache_blender.py","--","--source",str(source),"--output",str(root/"component.blend"),"--component-id",checked["component_id"]],capture_output=True,text=True,timeout=12*60)
