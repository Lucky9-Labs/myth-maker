"""Bounded Astra coordinator for the immutable component production graph."""
from __future__ import annotations
import base64, hashlib, json, re, time
from datetime import datetime, timezone
from pathlib import Path

FORMAT="myth-maker.component-coordinator-job/v1"; MODEL="gpt-6-astra"; NAME=re.compile(r"^[a-z0-9][a-z0-9-]{0,95}$")
...
def validate_component_coordinator_job(value):
    required={"format","run_id","work_id","attempt","model","objective","state_summary","evidence","prepared_actions","max_actions"}
    if not isinstance(value,dict) or set(value)!=required or value.get("format")!=FORMAT or value.get("model")!=MODEL: raise ValueError("component coordinator job has an invalid closed shape")
...
def run_component_coordinator(job,submissions_root,client):
    ...
    started=datetime.now(timezone.utc); clock=time.monotonic(); response=client.responses.create(model=MODEL,input=[{"role":"user","content":content}],reasoning={"effort":"medium"},text={"format":{"type":"json_schema","name":"component_coordinator_decision","strict":True,"schema":SCHEMA}},max_output_tokens=3000,timeout=300)
