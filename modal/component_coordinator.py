...
def run_component_coordinator(job,submissions_root,client):
    ...
    role_instruction=("Route already-classified work using state and prepared action metadata. Do not make visual-quality claims. " if checked["role"]=="routing" else "Judge visual evidence and choose the bounded actions that best improve reference fidelity and integration. For every multiview sheet, compare all views for identical topology before approving 3D generation. Reject the sheet when any opening, closed hub, recess, socket, panel boundary, part count, thickness, or attachment interface changes between views. Dark material and shadow are not holes. Required mating surfaces must be visibly planar and unobstructed. ")
