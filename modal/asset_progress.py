...
def accepted_section_revisions(run_root: Path) -> list[dict]:
    """Return the newest accepted immutable assembly for each stitch work id."""
    latest = {}
    for path in run_root.glob("agentic-stitch/*/attempt-*/receipt.json"):
        receipt = _read_json(path)
        if (not receipt or receipt.get("format") != "myth-maker.agentic-stitch-receipt/v1"
                or receipt.get("status") != "completed"
                or (receipt.get("visual_review") or {}).get("decision") != "accept"):
            continue
        work_id = str(receipt.get("work_id") or path.parent.parent.name)
        prior = latest.get(work_id)
        if prior is None or int(receipt.get("attempt") or 0) > int(prior["receipt"].get("attempt") or 0):
            latest[work_id] = {"receipt": receipt, "root": path.parent}
    return [latest[key] for key in sorted(latest)]

def latest_whole_body_scaffold(run_root: Path) -> dict | None:
    """Return the newest hash-verified pure assembly review scaffold."""
    candidates = []
    for path in run_root.glob("pure-assembly/*/attempt-*/receipt.json"):
        receipt = _read_json(path)
        artifact = (receipt or {}).get("artifacts", {}).get("three-quarter.png", {})
        render = path.parent / "three-quarter.png"
        if not receipt or receipt.get("status") != "completed" or not render.is_file():
            continue
        data = render.read_bytes()
        if len(data) != artifact.get("bytes") or hashlib.sha256(data).hexdigest() != artifact.get("sha256"):
            continue
        candidates.append((receipt.get("completed_at") or "", int(receipt.get("attempt") or 0), receipt, render))
    if not candidates:
        return None
    _completed, _attempt, receipt, render = max(candidates)
    return {"receipt": receipt, "render": render}
...
def build_dashboard(run_root: Path) -> dict:
    ...
    component_cards = []
    scaffold = latest_whole_body_scaffold(run_root)
    if scaffold:
        receipt = scaffold["receipt"]
        destination = output / "whole-body-scaffold.png"
        destination.write_bytes(scaffold["render"].read_bytes())
        artifact = receipt["artifacts"]["three-quarter.png"]
        component_cards.append(f'<section class="component-card" data-lifecycle="scaffold"><h3>Whole-body composition scaffold</h3><p class="badge scaffold">NONCANONICAL COMPOSITION</p><p class="muted">Best available samples arranged to expose whole-body silhouette and the next weakest region. Proximity here does not prove attachment, articulation, or promotion.</p><p class="hash">{html.escape(str(receipt.get("work_id") or "unknown"))} · attempt {int(receipt.get("attempt") or 0)} · {html.escape(str(receipt.get("completed_at") or "time unavailable"))}</p><img src="{destination.name}?sha={artifact["sha256"]}" alt="latest whole-body composition scaffold"></section>')
    canonical_sections = accepted_section_revisions(run_root)
    for item in canonical_sections:
        receipt = item["receipt"]
        artifact = (receipt.get("artifacts") or {}).get("three-quarter.png") or {}
        source = item["root"] / "three-quarter.png"
        if not source.is_file():
            continue
        destination = output / f'canonical-{receipt["work_id"]}.png'
        destination.write_bytes(source.read_bytes())
        assembly = (receipt.get("artifacts") or {}).get("assembly.glb") or {}
        scores = (receipt.get("visual_review") or {}).get("scores") or {}
        score_rows = "".join(f"<li>{html.escape(key.replace('_',' '))}: {value:.0f}</li>" for key,value in scores.items())
        component_cards.append(f'<section class="component-card" data-lifecycle="canonical"><h3>{html.escape(receipt["work_id"])}</h3><p class="badge canonical">CANONICAL ACCEPTED SECTION</p><p class="muted">Immutable section assembly accepted by Astra. Its source candidates remain candidates; this assembly hash is the promoted artifact.</p><p class="hash">GLB {html.escape(str(assembly.get("sha256") or "hash unavailable")[:12])}</p><img src="{destination.name}?sha={artifact.get("sha256", "")}" alt="{html.escape(receipt["work_id"])} accepted section"><ul>{score_rows}</ul></section>')
    ...
    scaffold_selected = "true" if scaffold else "false"
    canonical_selected = "false" if scaffold else "true"
    page = """<!doctype html><html><head><meta charset="utf-8"><meta http-equiv="refresh" content="300"><meta name="viewport" content="width=device-width"><title>Asset production progress</title><style>body{margin:0;background:#07111f;color:#e5eefb;font:15px system-ui;padding:24px}main{max-width:1200px;margin:auto}.grid,.metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:20px}section,.metric{background:#111c2d;padding:16px;border-radius:12px;margin:18px 0}img{width:100%;border-radius:8px;background:#030712}small,.muted{color:#9fb1c8}li{margin:.4em 0}table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:8px;border-bottom:1px solid #26364d}.bad{color:#fda4af}.good{color:#86efac}.active{color:#7dd3fc}.candidate{color:#fde68a}.canonical{color:#c4b5fd}.scaffold{color:#f9a8d4}.badge{font-weight:800;letter-spacing:.04em}.hash{font:12px ui-monospace;color:#a5b4fc}.filters{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0 18px}.filters button{border:1px solid #385070;background:#16263d;color:#dbeafe;padding:9px 13px;border-radius:999px;cursor:pointer}.filters button[aria-selected=true]{background:#2563eb;border-color:#60a5fa;color:white}.component-card[hidden]{display:none}</style></head><body><main><h1>Cloud asset production progress</h1><small>Run """ + html.escape(run_root.name) + " · generated " + html.escape(generated) + " · refreshes every 5 minutes</small><div class=\"metrics\"><div class=\"metric\"><b>Measured API spend</b><br>$" + f'{spend["measured_model_cost_usd"]:.4f}' + "</div><div class=\"metric\"><b>Accepted gain / $</b><br>" + str(efficiency["accepted_quality_gain_per_usd"] if efficiency["accepted_quality_gain_per_usd"] is not None else "unavailable") + "</div><div class=\"metric\"><b>Attempts</b><br>" + str(attempts["completed"]) + " completed / " + str(attempts["failed"]) + " failed</div><div class=\"metric\"><b>Open blockers</b><br><span class=\"bad\">" + str(quality["blocking_open"]) + "</span></div><div class=\"metric\"><b>Quality gain / compute minute</b><br>" + str(efficiency["quality_gain_per_compute_minute"] if efficiency["quality_gain_per_compute_minute"] is not None else "unavailable") + "</div></div>" + spend_chart_html + chart_html + "<section><h2>How to read component status</h2><p><b>Canonical</b> is reserved for a hash promoted by an accepted section or final asset receipt. Source components used to build it remain candidates because stitching may rebuild their geometry.</p><ul><li><span class=\"scaffold\"><b>NONCANONICAL COMPOSITION</b></span>: current whole-body arrangement used to choose the next replacement.</li><li><span class=\"canonical\"><b>CANONICAL ACCEPTED SECTION</b></span>: immutable assembly that passed the section gate.</li><li><span class=\"active\"><b>ACTIVE STITCH INPUT</b></span>: selected for the newest section job; it may be rebuilt during stitching.</li><li><span class=\"good\"><b>STITCH-READY CANDIDATE</b></span>: passed component review but awaits section acceptance.</li><li><span class=\"candidate\"><b>CLEANUP CANDIDATE</b></span>: useful shape requiring repair.</li><li><span class=\"bad\"><b>REJECTED</b></span>: cannot enter canonical assembly unchanged.</li></ul></section><section><h2>Asset patches in the last 5 minutes</h2><table><thead><tr><th>Lane</th><th>Status</th><th>Attempt</th><th>Output hashes</th><th>Completed</th></tr></thead><tbody>" + patch_rows + "</tbody></table></section><section><h2>Measured model spend</h2><table><thead><tr><th>Model</th><th>Provenance</th><th>Requests</th><th>Uncached input USD</th><th>Cached input USD</th><th>Output USD</th><th>Total USD</th></tr></thead><tbody>" + model_rows + "</tbody></table><p class=\"muted\">Each dollar amount is calculated from recorded usage and that model's published rates at the time of the call. Raw usage counts remain in the machine-readable manifest for auditability. Blender scripts use compute time and zero model calls. Models with unavailable task usage remain unpriced rather than counted as free.</p></section><h2>Assembly and component lifecycle</h2><nav class=\"filters\" aria-label=\"Assembly and component lifecycle filter\"><button type=\"button\" data-filter=\"scaffold\" aria-selected=\"" + scaffold_selected + "\">Whole-body scaffold</button><button type=\"button\" data-filter=\"canonical\" aria-selected=\"" + canonical_selected + "\">Canonical</button><button type=\"button\" data-filter=\"active\" aria-selected=\"false\">Active</button><button type=\"button\" data-filter=\"good\" aria-selected=\"false\">Stitch-ready</button><button type=\"button\" data-filter=\"candidate\" aria-selected=\"false\">Cleanup</button><button type=\"button\" data-filter=\"bad\" aria-selected=\"false\">Rejected</button><button type=\"button\" data-filter=\"all\" aria-selected=\"false\">All</button></nav><p id=\"empty-filter\" class=\"muted\" hidden>No assemblies or components currently have this status.</p><div id=\"component-grid\" class=\"grid\">" + "".join(component_cards) + "</div><h2>Last four completed developments</h2><div class=\"grid\">" + "".join(cards) + "</div></main><script>(()=>{const buttons=[...document.querySelectorAll('[data-filter]')],cards=[...document.querySelectorAll('.component-card')],empty=document.getElementById('empty-filter');function apply(value){let shown=0;for(const card of cards){const visible=value==='all'||card.dataset.lifecycle===value;card.hidden=!visible;if(visible)shown++}empty.hidden=shown>0;for(const button of buttons)button.setAttribute('aria-selected',String(button.dataset.filter===value));location.hash='components-'+value}for(const button of buttons)button.addEventListener('click',()=>apply(button.dataset.filter));const requested=location.hash.replace('#components-','');const fallback=cards.some(card=>card.dataset.lifecycle==='scaffold')?'scaffold':'canonical';apply(['scaffold','canonical','active','good','candidate','bad','all'].includes(requested)?requested:fallback)})()</script></body></html>"
    ...
    manifest = {"format": "myth-maker.asset-progress-dashboard/v2", "run_id": run_root.name, "generated_at": generated, "refresh_seconds": 300, "telemetry": telemetry, "assets": assets,
                ...
                "whole_body_scaffold": ({k: v for k, v in scaffold["receipt"].items()} if scaffold else None),
                "canonical_sections": [{"work_id": item["receipt"].get("work_id"),
                    "attempt": item["receipt"].get("attempt"), "artifacts": item["receipt"].get("artifacts", {})}
                    for item in canonical_sections],
