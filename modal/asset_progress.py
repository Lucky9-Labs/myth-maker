...
def build_dashboard(run_root: Path) -> dict:
    ...
    for item in component_reviews:
        ...
        component_cards.append(f'<section><h3>{html.escape(item["component_id"])}</h3><p class="badge {lifecycle_class}">{lifecycle}</p><p class="muted">{html.escape(lifecycle_reason)}</p><p class="hash">Candidate {html.escape(str(item.get("candidate_sha256") or "hash unavailable")[:12])}</p><img src="{destination.name}?sha={item["artifacts"]["three-quarter.png"]["sha256"]}" alt="{html.escape(item["component_id"])} diagnostic render"><p><b>Astra routing:</b> {html.escape(review["decision"])}</p><ul>{score_rows}</ul><b>Top blockers</b><ul>{defect_rows}</ul></section>')
