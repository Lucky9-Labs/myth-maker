...
test("the provider interface has separate least-privilege credentials", () => {
  ...
  assert.deepEqual(providerDefinitions.cloudflare.secretNames, ["CLOUDFLARE_API_TOKEN", "AGENT_INGRESS_TOKEN", "PACKAGE_DISCOVERY_SIGNING_PRIVATE_KEY", "CATALOG_ACCEPTANCE_TOKEN", "WORK_DISPATCH_TOKEN"]);
  assert.deepEqual(providerDefinitions.cloudflare.plainVariableNames, ["WORK_DISPATCH_URL"]);
...
test("Cloudflare receipts require the deployed current Worker version and bound secret names", () => {
  ...
  assert.deepEqual(parseCloudflareDeploymentEvidence(JSON.stringify({
    ...
    plain_bindings: { WORK_DISPATCH_URL: "https://dispatcher.example/v1/dispatch" },
  })), {
    version_id: versionId,
    worker_url: "https://myth-maker-encounter-runtime.example.workers.dev",
    secret_bindings: ["AGENT_INGRESS_TOKEN", "CATALOG_ACCEPTANCE_TOKEN", "PACKAGE_DISCOVERY_SIGNING_PRIVATE_KEY", "WORK_DISPATCH_TOKEN"],
...
test("Cloudflare receipt parsing selects the latest 100-percent deployment rather than list position", () => {
  const versionId = "05825930-6538-41a1-b0f9-26f20f06a34b";
  const olderVersionId = "11111111-1111-4111-8111-111111111111";
  const deployments = [
    { id: "old-deployment", created_on: "2026-09-09T01:20:00.000Z", versions: [{ version_id: olderVersionId, percentage: 100 }] },
    { id: "9da9107a-4b45-478b-8a34-5437e5575709", created_on: "2026-09-09T01:27:09.000Z", versions: [{ version_id: versionId, percentage: 100 }] },
  ];
  const versions = [{ id: olderVersionId }, { id: versionId }];
  assert.equal(currentCloudflareVersionId(JSON.stringify(deployments), JSON.stringify(versions), `Current Version ID: ${versionId}`), versionId);
});
...
test("workflows use non-mutating PR previews and provider locks", async () => {
  ...
  assert.deepEqual(cloudflareConfig.secrets.required, ["AGENT_INGRESS_TOKEN", "PACKAGE_DISCOVERY_SIGNING_PRIVATE_KEY", "CATALOG_ACCEPTANCE_TOKEN", "WORK_DISPATCH_TOKEN"]);
  assert.equal(cloudflareConfig.vars.WORK_DISPATCH_URL, "");
  for (const job of [
