...
test("the committed Unity v2 selected-manifest fixture verifies over canonical unsigned JSON", async () => {
  const fixture = JSON.parse(await readFile(new URL("./fixtures/package-discovery-v2-unity-selected.fixture.json", import.meta.url), "utf8"));
  const unsignedManifest = structuredClone(fixture.manifest);
  delete unsignedManifest.signature;
  const publicKey = await crypto.subtle.importKey("spki", Buffer.from(fixture.public_key_spki_base64, "base64"), { name: "Ed25519" }, false, ["verify"]);
  assert.equal(await crypto.subtle.verify({ name: "Ed25519" }, publicKey, Buffer.from(fixture.manifest.signature.value, "base64url"), new TextEncoder().encode(canonicalJson(unsignedManifest))), true);
});
