...
export function parseCloudflareDeploymentEvidence(output) {
  ...
  if (!UUID.test(parsed?.version_id ?? "") || typeof parsed?.worker_url !== "string" || !parsed.worker_url.endsWith(".workers.dev")
    ...
    || !Array.isArray(parsed?.runtime_variables)
    || parsed.runtime_variables.length !== CLOUDFLARE_RUNTIME_VARIABLE_BINDINGS.length
    || [...parsed.runtime_variables].sort().join(",") !== [...CLOUDFLARE_RUNTIME_VARIABLE_BINDINGS].sort().join(",")) {
    throw new Error("Cloudflare deployment evidence requires the current Worker version and all runtime bindings");
  }
  return {
    version_id: parsed.version_id,
    worker_url: parsed.worker_url,
    secret_bindings: [...parsed.secret_bindings].sort(),
    runtime_variables: [...parsed.runtime_variables].sort(),
  };
}
...
function executeCloudflareDeployment(invocation) {
  ...
  const dispatchUrl = process.env.WORK_DISPATCH_URL;
  if (typeof dispatchUrl !== "string" || !/^https:\/\/[^/]+\/v1\/dispatch$/.test(dispatchUrl)) {
    throw new Error("WORK_DISPATCH_URL must be an HTTPS /v1/dispatch endpoint");
  }
  ...
  const deployOutput = cloudflareCommand(["deploy", "--var", `WORK_DISPATCH_URL:${dispatchUrl}`]);
  ...
  return {
    ...
    runtime_variables: [...CLOUDFLARE_RUNTIME_VARIABLE_BINDINGS],
  };
}
