...
def main() -> int:
    ...
    if provider.get("provider") != "github-actions-runner" or not re.fullmatch(
        r"[a-z0-9][a-z0-9-]{0,127}", job_id
    ):
        raise RuntimeError("worker receipt has no trusted runner job ID")
